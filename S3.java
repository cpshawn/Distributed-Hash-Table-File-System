...
public class S3 {
    static String serverName = "S3";    // Node name of this server, which is the same as class name
    static int serverId = 3;            // Server id number
    ...
    static Boolean[] portStatus = {false, false, false, true, false, false, false};     // Port status, to simulate disabled and enabled channels
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket S3ReceiverSocket = new ServerSocket(2003)) {
            ...
            String S3Address = InetAddress.getLocalHost().getHostAddress();
            System.out.println("Server S3 online. IP address: " + S3Address + ". Assigned to group " + groupId);
            ...
            ServerHelper.updateServerAddressesTXT(serverName, S3Address);
            ...
            Socket S3SenderToS0 = new Socket(S0Address, ServerHelper.portNums.get(0));
            otherSockets.add(S3SenderToS0);
            ServerHelper.ReceiverThread S0receiverThread = new ServerHelper.ReceiverThread("S3SenderToS0", lamportTimestamp, S3SenderToS0, ReceivedMessages);
            S0receiverThread.start();
            portStatus[0] = true;
            System.out.println("Established connection to server S0.");

            Socket S3SenderToS1 = new Socket(S1Address, ServerHelper.portNums.get(1));
            otherSockets.add(S3SenderToS1);
            ServerHelper.ReceiverThread S1receiverThread = new ServerHelper.ReceiverThread("S3SenderToS1", lamportTimestamp, S3SenderToS1, ReceivedMessages);
            S1receiverThread.start();
            portStatus[1] = true;
            System.out.println("Established connection to server S1.");

            Socket S3SenderToS2 = new Socket(S2Address, ServerHelper.portNums.get(2));
            otherSockets.add(S3SenderToS2);
            ServerHelper.ReceiverThread S2receiverThread = new ServerHelper.ReceiverThread("S3SenderToS2", lamportTimestamp, S3SenderToS2, ReceivedMessages);
            S2receiverThread.start();
            portStatus[2] = true;
            System.out.println("Established connection to server S2.");

            Socket S3SenderToS4 = S3ReceiverSocket.accept();
            otherSockets.add(S3SenderToS4);
            ServerHelper.ReceiverThread S4receiverThread = new ServerHelper.ReceiverThread("S3SenderToS4", lamportTimestamp, S3SenderToS4, ReceivedMessages);
            S4receiverThread.start();
            portStatus[4] = true;
            System.out.println("Established connection to server S4.");

            Socket S3SenderToS5 = S3ReceiverSocket.accept();
            otherSockets.add(S3SenderToS5);
            ServerHelper.ReceiverThread S5receiverThread = new ServerHelper.ReceiverThread("S3SenderToS5", lamportTimestamp, S3SenderToS5, ReceivedMessages);
            S5receiverThread.start();
            portStatus[5] = true;
            System.out.println("Established connection to server S5.");

            Socket S3SenderToS6 = S3ReceiverSocket.accept();
            otherSockets.add(S3SenderToS6);
            ServerHelper.ReceiverThread S6receiverThread = new ServerHelper.ReceiverThread("S3SenderToS6", lamportTimestamp, S3SenderToS6, ReceivedMessages);
            S6receiverThread.start();
            portStatus[6] = true;
            System.out.println("Established connection to server S6.");

            Socket C0 = S3ReceiverSocket.accept();
            otherSockets.add(C0);
            ...
            Socket C1 = S3ReceiverSocket.accept();
            otherSockets.add(C1);
            ...
            Socket C2 = S3ReceiverSocket.accept();
            otherSockets.add(C2);
            ...
            Socket C3 = S3ReceiverSocket.accept();
            otherSockets.add(C3);
            ...
            Socket C4 = S3ReceiverSocket.accept();
            otherSockets.add(C4);
