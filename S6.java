...
public class S6 {
    static String serverName = "S6";    // Node name of this server, which is the same as class name
    static int serverId = 6;            // Server id number
    ...
    static Boolean[] portStatus = {false, false, false, false, false, false, true};     // Port status, to simulate disabled and enabled channels
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket S6ReceiverSocket = new ServerSocket(2006)) {
            ...
            String S6Address = InetAddress.getLocalHost().getHostAddress();
            System.out.println("Server S6 online. IP address: " + S6Address + ". Assigned to group " + groupId);
            ...
            ServerHelper.updateServerAddressesTXT(serverName, S6Address);
            ...
            String S5Address = serverAddresses.get(5).trim().split(": ")[1];

            Socket S6SenderToS0 = new Socket(S0Address, ServerHelper.portNums.get(0));
            otherSockets.add(S6SenderToS0);
            ServerHelper.ReceiverThread S0receiverThread = new ServerHelper.ReceiverThread("S6SenderToS0", lamportTimestamp, S6SenderToS0, ReceivedMessages);
            S0receiverThread.start();
            portStatus[0] = true;
            System.out.println("Established connection to server S0.");

            Socket S6SenderToS1 = new Socket(S1Address, ServerHelper.portNums.get(1));
            otherSockets.add(S6SenderToS1);
            ServerHelper.ReceiverThread S1receiverThread = new ServerHelper.ReceiverThread("S6SenderToS1", lamportTimestamp, S6SenderToS1, ReceivedMessages);
            S1receiverThread.start();
            portStatus[1] = true;
            System.out.println("Established connection to server S1.");

            Socket S6SenderToS2 = new Socket(S2Address, ServerHelper.portNums.get(2));
            otherSockets.add(S6SenderToS2);
            ServerHelper.ReceiverThread S2receiverThread = new ServerHelper.ReceiverThread("S6SenderToS2", lamportTimestamp, S6SenderToS2, ReceivedMessages);
            S2receiverThread.start();
            portStatus[2] = true;
            System.out.println("Established connection to server S2.");

            Socket S6SenderToS3 = new Socket(S3Address, ServerHelper.portNums.get(3));
            otherSockets.add(S6SenderToS3);
            ServerHelper.ReceiverThread S3receiverThread = new ServerHelper.ReceiverThread("S6SenderToS3", lamportTimestamp, S6SenderToS3, ReceivedMessages);
            S3receiverThread.start();
            portStatus[3] = true;
            System.out.println("Established connection to server S3.");

            Socket S6SenderToS4 = new Socket(S4Address, ServerHelper.portNums.get(4));
            otherSockets.add(S6SenderToS4);
            ServerHelper.ReceiverThread S4receiverThread = new ServerHelper.ReceiverThread("S6SenderToS4", lamportTimestamp, S6SenderToS4, ReceivedMessages);
            S4receiverThread.start();
            portStatus[4] = true;
            System.out.println("Established connection to server S4.");

            Socket S6SenderToS5 = new Socket(S5Address, ServerHelper.portNums.get(5));
            otherSockets.add(S6SenderToS5);
            ServerHelper.ReceiverThread S5receiverThread = new ServerHelper.ReceiverThread("S6SenderToS5", lamportTimestamp, S6SenderToS5, ReceivedMessages);
            S5receiverThread.start();
            portStatus[5] = true;
            System.out.println("Established connection to server S5.");

            Socket C0 = S6ReceiverSocket.accept();
            otherSockets.add(C0);
            ...
            Socket C1 = S6ReceiverSocket.accept();
            otherSockets.add(C1);
            ...
            Socket C2 = S6ReceiverSocket.accept();
            otherSockets.add(C2);
            ...
            Socket C3 = S6ReceiverSocket.accept();
            otherSockets.add(C3);
            ...
            Socket C4 = S6ReceiverSocket.accept();
            otherSockets.add(C4);
