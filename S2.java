...
public class S2 {
    static String serverName = "S2";    // Node name of this server, which is the same as class name
    static int serverId = 2;            // Server id number
    ...
    static Boolean[] portStatus = {false, false, true, false, false, false, false};     // Port status, to simulate disabled and enabled channels
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket S2ReceiverSocket = new ServerSocket(2002)) {
            ...
            String S2Address = InetAddress.getLocalHost().getHostAddress();
            System.out.println("Server S2 online. IP address: " + S2Address + ". Assigned to group " + groupId);
            ...
            ServerHelper.updateServerAddressesTXT(serverName, S2Address);
            ...
            Socket S2SenderToS0 = new Socket(S0Address, ServerHelper.portNums.get(0));
            otherSockets.add(S2SenderToS0);
            ServerHelper.ReceiverThread S0receiverThread = new ServerHelper.ReceiverThread("S2SenderToS0", lamportTimestamp, S2SenderToS0, ReceivedMessages);
            S0receiverThread.start();
            portStatus[0] = true;
            System.out.println("Established connection to server S0.");

            Socket S2SenderToS1 = new Socket(S1Address, ServerHelper.portNums.get(1));
            otherSockets.add(S2SenderToS1);
            ServerHelper.ReceiverThread S1receiverThread = new ServerHelper.ReceiverThread("S2SenderToS1", lamportTimestamp, S2SenderToS1, ReceivedMessages);
            S1receiverThread.start();
            portStatus[1] = true;
            System.out.println("Established connection to server S1.");

            Socket S2SenderToS3 = S2ReceiverSocket.accept();
            otherSockets.add(S2SenderToS3);
            ServerHelper.ReceiverThread S3receiverThread = new ServerHelper.ReceiverThread("S2SenderToS3", lamportTimestamp, S2SenderToS3, ReceivedMessages);
            S3receiverThread.start();
            portStatus[3] = true;
            System.out.println("Established connection to server S3.");

            Socket S2SenderToS4 = S2ReceiverSocket.accept();
            otherSockets.add(S2SenderToS4);
            ServerHelper.ReceiverThread S4receiverThread = new ServerHelper.ReceiverThread("S2SenderToS4", lamportTimestamp, S2SenderToS4, ReceivedMessages);
            S4receiverThread.start();
            portStatus[4] = true;
            System.out.println("Established connection to server S4.");

            Socket S2SenderToS5 = S2ReceiverSocket.accept();
            otherSockets.add(S2SenderToS5);
            ServerHelper.ReceiverThread S5receiverThread = new ServerHelper.ReceiverThread("S2SenderToS5", lamportTimestamp, S2SenderToS5, ReceivedMessages);
            S5receiverThread.start();
            portStatus[5] = true;
            System.out.println("Established connection to server S5.");

            Socket S2SenderToS6 = S2ReceiverSocket.accept();
            otherSockets.add(S2SenderToS6);
            ServerHelper.ReceiverThread S6receiverThread = new ServerHelper.ReceiverThread("S2SenderToS6", lamportTimestamp, S2SenderToS6, ReceivedMessages);
            S6receiverThread.start();
            portStatus[6] = true;
            System.out.println("Established connection to server S6.");

            Socket C0 = S2ReceiverSocket.accept();
            otherSockets.add(C0);
            ...
            Socket C1 = S2ReceiverSocket.accept();
            otherSockets.add(C1);
            ...
            Socket C2 = S2ReceiverSocket.accept();
            otherSockets.add(C2);
            ...
            Socket C3 = S2ReceiverSocket.accept();
            otherSockets.add(C3);
            ...
            Socket C4 = S2ReceiverSocket.accept();
            otherSockets.add(C4);
